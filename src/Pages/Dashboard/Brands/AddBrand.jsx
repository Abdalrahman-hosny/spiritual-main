...
const AddBrand = ({ onClose, onBrandAdded }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.code === 200) {
        ...
        onBrandAdded(response.data.data);
      } else {
        toast.error(
          response.data.message ||
            (i18n.language === "ar"
              ? "خطأ في إضافة البراند"
              : "Error adding brand")
        );
      }
    } catch (error) {
      ...
      toast.error(
        i18n.language === "ar" ? "خطأ في إضافة البراند" : "Error adding brand"
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className={styles.modalOverlay} onClick={onClose}>
      ...
      >
        <div className={styles.modalHeader}>
          <h2>
            {i18n.language === "ar" ? "إضافة براند جديد" : "Add New Brand"}
          </h2>
