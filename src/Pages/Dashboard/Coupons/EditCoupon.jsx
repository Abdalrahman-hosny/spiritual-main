...
const EditCoupon = ({ coupon, onClose, onCouponUpdated }) => {
  ...
  useEffect(() => {
    if (coupon) {
      setFormData({
        code: coupon.code || "",
        discount_precentage: coupon.discount_precentage || "",
        discount_type: coupon.discount_type || "percentage",
        start_date: coupon.start_date || "",
        end_date: coupon.end_date || "",
        limit: coupon.limit || "",
        time_used: coupon.time_used || "0",
      });
    }
  }, [coupon]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `https://spiritual.brmjatech.uk/api/dashboard/coupon/${coupon.id}`,
        submitData,
        {
          headers: {
    ...
    <div className={styles.modalOverlay} onClick={onClose}>
      ...
      >
        <div className={styles.modalHeader}>
          <h2>{i18n.language === "ar" ? "تعديل الكوبون" : "Edit Coupon"}</h2>
